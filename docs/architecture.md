...
6 层缓存架构管理器，支持 Anthropic 和 OpenAI 提示缓存。每层作为独立的 `SystemBlock`，Anthropic 原生支持多 block 数组（带 `cache_control`），OpenAI/Ollama 在适配层拼接为单个 system message。

```go
// PromptCacheManager 管理缓存层
type PromptCacheManager struct {
    staticSystem     string  // Layer 0: 静态系统提示
    profileBlock     string  // Layer 1: 用户 profile markdown
    toolsBlock       string  // Layer 2: 工具定义
    skillsBlock      string  // Layer 3: 技能元数据
    agentsBlock      string  // Layer 4: Agent 元数据
    projectAuraBlock string  // Layer 5: 项目 AURA.md
}

// 缓存层常量
const (
    LayerStaticSystem  CacheLayer = 0
    LayerProfile       CacheLayer = 1
    LayerTools         CacheLayer = 2
    LayerSkills        CacheLayer = 3
    LayerAgents        CacheLayer = 4
    LayerProjectAura   CacheLayer = 5
)
...
```
初始化（boot.go）:
  PromptCacheManager 存储 6 层 string
  BuildSystemBlocks() → []llm.SystemBlock（每层带 cache_control: ephemeral）
  → PromptCacheConfig.SystemBlocks 传入 Engine

每轮请求（engine/action_parser.go）:
  静态内容 → PromptCacheConfig.SystemBlocks（6 层 blocks，初始化时构建）
  动态内容 → Messages[] 中 role=system 消息（RAG/summary/skill bodies，每轮变化）

Provider 适配（最底层）:
  Anthropic: 静态 blocks + 动态 messages → []systemBlock（原生数组，前缀缓存）
             静态 blocks 带 cache_control，动态 messages 不带
  OpenAI:    SystemBlocks 拼接为单个 system message + Messages 中 system 保留
  Ollama:    同 OpenAI
```
...
6 层缓存架构，支持 Anthropic 和 OpenAI 提示缓存：
- **Layer 0**: StaticSystem — 静态系统提示（最高缓存率）
- **Layer 1**: Profile — 用户 profile markdown
- **Layer 2**: Tools — 工具定义
- **Layer 3**: Skills — 技能元数据
- **Layer 4**: Agents — Agent 元数据
- **Layer 5**: ProjectAura — 项目 AURA.md
- **静态 vs 动态**：
  - 静态内容（6 层）通过 `PromptCacheConfig.SystemBlocks` 传递，初始化时构建，带 `cache_control`
  - 动态内容（RAG、对话摘要、激活的 skill bodies）通过 `Messages[]` 中 `role=system` 消息传递，每轮变化
- **Provider 实现**：
  - Anthropic: `SystemBlocks` + 动态 messages → `[]systemBlock` 数组（原生多 block，前缀缓存）
  - OpenAI: `SystemBlocks` 拼接为单个 system message + `cache_type: "ephemeral"` 在请求级
