...
**详细文档：** [Skill System](modules/skill/README.md)
...
│   │   ├── pkg/agent/          # AgentMeta 定义（YAML frontmatter）
...
│   │   ├── pkg/constants/      # 常量定义
│   │   ├── pkg/loader/         # 配置文件加载
│   │   ├── pkg/client/         # MCP stdio 客户端
│   │   ├── pkg/tool/           # MCPTool 适配器（实现 tools.Tool）
│   │   └── pkg/manager/        # MCP 服务器生命周期管理
...
│   │   │   ├── delegate.go     # Agent 委托功能
│   │   │   └── events.go       # 事件定义
│   │   ├── pkg/intent/         # Core 层意图识别服务
│   │   ├── pkg/sdk/            # SDK 导出层
│   │   ├── pkg/planner/        # 任务规划器
│   │   ├── pkg/memory/         # 对话记忆系统（令牌感知、摘要生成）
│   │   ├── pkg/permissions/    # 权限管理器
│   │   ├── pkg/orchestrator/   # 多 Agent 编排器
│   │   ├── pkg/workspace/      # 工作空间隔离器
│   │   ├── pkg/factory/        # 工厂函数
│   │   ├── pkg/llm/            # LLM 客户端 (Ollama, OpenAI)
│   │   └── pkg/prompt/         # Prompt 构建器
...
| `commands` | `github.com/oneliang/aura/commands` | `knowledge`, `personality`, `session`, `shared` |
| `core` | `github.com/oneliang/aura/core` | `agent`, `commands`, `session`, `shared`, `storage`, `tools` |
| `api` | `github.com/oneliang/aura/api` | `core`, `session` |
| `adapters` | `github.com/oneliang/aura/adapters` | `core`, `session`, `shared` |
| `cli` | `github.com/oneliang/aura/cli` | `agent`, `api`, `commands`, `core`, `knowledge`, `personality`, `session`, `tools`, `shared`, `habit` |
...
| `core/pkg/llm` | `github.com/oneliang/aura/core/pkg/llm` | LLM 客户端 (Ollama、OpenAI、Anthropic) |
| `core/pkg/prompt` | `github.com/oneliang/aura/core/pkg/prompt` | Prompt 构建器 (系统提示、角色加载) |
| `core/pkg/intent` | `github.com/oneliang/aura/core/pkg/intent` | 意图识别服务 (自然语言命令识别) |

### 核心接口

**Engine** (`modules/core/pkg/engine/engine.go`):
```go
type Engine struct {
    client          llm.Client
...
**Message** (`modules/shared/pkg/memory/memory.go`):
...
// AgentConfig (modules/shared/pkg/config/config.go)
...
**Orchestrator** (`modules/core/pkg/orchestrator/orchestrator.go`):
```go
...
**AgentManager** (`modules/agent/pkg/manager/manager.go`):
...
**SkillManager** (`modules/skill/pkg/manager/manager.go`):
...
**IntentRecognizer** (`modules/commands/pkg/intent/recognizer.go`):
```go
...
**AliasManager** (`modules/commands/pkg/alias/alias.go`):
```go
type Manager struct {
...
**Event Bus** (`modules/shared/pkg/events/bus.go`):
```go
// Bus 提供事件发布/订阅功能
type Bus struct {
...
**IntentService** (`modules/core/pkg/intent/service.go`):
```go
...
**IntentResult** (`modules/core/pkg/intent/service.go`):
```go
...
**ThinkingWidget** (`modules/cli/pkg/tui/thinking.go`):
```go
...
**事件类型** (`modules/shared/pkg/events/events.go`):
```go
// Engine 执行事件
